...
def my_get_cosine_with_hard_restarts_schedule_with_warmup(
    optimizer: Optimizer, num_warmup_steps: int, num_training_steps: int, num_cycles: int = 1, last_epoch: int = -1, decay_factor = 0.8
):
    """
    ...
    def lr_lambda(current_step):
        ...
        return max(0.0, decay_factor ** (int(num_cycles * progress) // 1) * 0.5 * (1.0 + math.cos(math.pi * ((float(num_cycles) * progress) % 1.0))))
...
#                                     num_warmup_steps = 2 * len(train_dataloader), num_training_steps = len(train_dataloader) * self.epoch_num)
        
        
        # scheduler = my_get_cosine_with_hard_restarts_schedule_with_warmup(optimizer,
        #                              num_warmup_steps = len(train_dataloader) * 2, num_training_steps = len(train_dataloader) * self.epoch_num, num_cycles = 2, decay_factor = self.lr_decay)
        ...
        #scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer,T_0 = self.epoch_num // 5, T_mult = 3)
